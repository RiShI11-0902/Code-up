...
const QuizPage = () => {
  ...
  return (
    <>
      ...
      {
        p1 !== " " && p2 !== "" && gameOver == false ? <div className=' '>
          {
            <div className="md:w-full w-80 mt-28 md:mt-0 mx-auto bg-white shadow-md p-5 rounded-lg">
              {gameOver == true ? "Game Over" : " "}
              <p>{ index > questions.length - 1 ? "" : <p><span className='text-3xl -mt-5'>{index+1}</span><span>/{questions.length}</span></p> } </p>
              <h2  className="text-xl font-semibold mb-4 mt-3">{index > questions.length - 1 ?  <div className='flex flex-col items-center space-y-5 '>  Result Pending <RiLoader3Line className='animate-spin mt-5' /> </div> : questions[index]?.question}</h2>
        ...
        </div> : <div className=' question w-80 md:w-[30rem] h-fit mx-auto rounded-3xl bg-white p-5 '>
          {gameOver ?
            <div className='font-bold text-xl md:text-3xl'>
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 1, ease: 'easeInOut' }}  className="box text-center">
                {
